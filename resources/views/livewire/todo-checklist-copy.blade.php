...
<div>
    ...
    <?php
    use App\Models\Client; // Make sure to import your Client model
    use Illuminate\Support\Carbon;
    use Illuminate\Support\Facades\Auth;

    // Retrieve the wedding date for the authenticated user
    $weddingDate = Client::where('user_id', Auth::id())->value('wed_date');

    if ($weddingDate) {
        $weddingDate = Carbon::parse($weddingDate);

        // Calculate the difference in days between the current date and the wedding date
        $daysDifference = Carbon::now()->diffInDays($weddingDate);

        // Define an array of time periods in days
        $timePeriods = [
            'From 10 to 12 months' => [304, 365], // 10 months to 1 year
            'From 7 to 9 months' => [213, 273], // 7 months to 9 months
            'From 4 to 6 months' => [121, 182], // 4 months to 6 months
            'The last month' => [1, 30], // Last month
            '2 weeks' => [1, 14], // 2 weeks
            'Last week' => [1, 7], // Last week
            'Last day' => [0, 0], // Last day
            'After the wedding' => [1, PHP_INT_MAX], // Assuming wedding date is in the past
        ];

        // Determine the appropriate time period based on the difference in days
        $currentPeriod = '';
        foreach ($timePeriods as $period => $range) {
            if ($daysDifference >= $range[0] && $daysDifference <= $range[1]) {
                $currentPeriod = $period;
                break;
            }
        }

        // Format the "Before Month Year" string
        $beforeDate = $weddingDate->format('F Y');

        // Output the results
        echo '<h5 class="fw-bold">' . $currentPeriod . '</h5>';
        echo '<p class="fw-bold">Before ' . $beforeDate . '</p>';
    } else {
        // Handle the case where there's no wedding date for the user
        echo 'No wedding date found.';
    }
    ?>
