...
class Clientregister extends Component implements Forms\Contracts\HasForms
{
                ...
                ->schema([
                  ...
                  TextInput::make('c_location')
                    ...
                    ->schema([
                        ...
                        TimePickerField::make('wed_end_time')->label('Wedding End Time')->okLabel("Confirm")->cancelLabel("Cancel"),
                        ]),
    ...
    {
        if(Auth::user()->id){
            $client = DB::table('clients')->where('user_id', Auth::user()->id)->first();
        }
        ...
        if($client){
            ...
            return redirect()->route('dashboard')->with('error','You already has registered');
        }else{
            Notification::make()
